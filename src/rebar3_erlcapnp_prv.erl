-module(rebar3_erlcapnp_prv).


-export([init/1, do/1, format_error/1]).

-define(PROVIDER, compile).
-define(NAMESPACE, erlcapnp).
-define(DEPS, []).
...
init(State) ->
    Provider = providers:create([
            ...
            {bare, true},
            {deps, ?DEPS},
            {example, "rebar3 erlcapnp compile"},
            {opts, []},
...
do(State) ->
    ConfigOpts = rebar_state:get(State, erlcapnp, []),
    rebar_api:info("Its working!!! ~p", [ConfigOpts]),
